const FgEmojiPicker=function(options){this.options=options;if(!this.options){return console.error('You must provide object as a first argument')}
this.init=()=>{this.functions.generateStyle();this.selectors.trigger=this.options.hasOwnProperty('trigger')?this.options.trigger:console.error('You must proved trigger element like this - \'EmojiPicker.init({trigger: "selector"})\' ');this.selectors.search='.fg-emoji-picker-search input';this.selectors.emojiContainer='.fg-emoji-picker-grid'
this.emojiItems=undefined;this.variable.emit=this.options.emit||null;this.variable.position=this.options.position||null;this.variable.dir=this.options.dir||'';if(!this.selectors.trigger)return;this.bindEvents();}
this.variable={position:null,dir:'',}
this.selectors={emit:null,trigger:null}
this.bindEvents=()=>{document.body.addEventListener('click',this.functions.removeEmojiPicker.bind(this));document.body.addEventListener('click',this.functions.emitEmoji.bind(this));document.body.addEventListener('click',this.functions.openEmojiSelector.bind(this),this.selectors.trigger);document.body.addEventListener('input',this.functions.search.bind(this),this.selectors.search);}
this.functions={search(e){const val=e.target.value;if(!Array.isArray(this.emojiItems)){this.emojiItems=Array.from(e.target.closest('.fg-emoji-picker').querySelectorAll('.fg-emoji-picker-all-categories li'));}
this.emojiItems.filter(emoji=>{if(!emoji.getAttribute('data-name').match(val)){emoji.style.display='none'}else{emoji.style.display=''}})
if(!val.length)this.emojiItems=undefined;},generateStyle(){document.head.insertAdjacentHTML('beforeend',`
                <style>
                .fg-emoji-picker {
                    /* position: fixed; */
                    position: absolute;
...
let picker=`
        ...
        </div>`;let categories='<ul>%categories%</ul>';let categoriesInner=``;let outerUl=`
        <div class="fg-emoji-picker-all-categories">%outerUL%</div>
        `;let innerLists=``;const fetchData=fetch(`asset/js/plugin/json/full-emoji-list.json`).then(response=>response.json()).then(object=>{let index=0;for(const key in object){if(object.hasOwnProperty(key)){index+=1;let keyToId=key.split(' ').join('-').split('&').join('').toLowerCase();const categories=object[key];categoriesInner+=`<li class="${index===1?'active':''}" id="${keyToId}" data-index="${keyToId}"><a href="#${keyToId}">${categoryIcons[keyToId]}</a></li>`
innerLists+=`
                        <ul class="fg-emoji-picker-category ${index===1?'active':''}" id="${keyToId}" category-name="${key}">
                            ...
                            <div class="fg-emoji-picker-grid">`;categories.forEach(item=>{innerLists+=`<li data-name="${item.description.toLowerCase()}"><a class="fg-emoji-picker-item" title="${item.description}" data-name="${item.description.toLowerCase()}" data-code="${item.code}" href="${item.emoji}">${item.emoji}</a></li>`;})
innerLists+=`
                            </div>
                        </ul>`;}}
let allSmiles=outerUl.replace('%outerUL%',innerLists)
let cats=categories.replace('%categories%',categoriesInner);let pickerContainer=picker.replace('%pickerContainer%',allSmiles)
let data=pickerContainer.replace('%categories%',cats);return data;})
return fetchData;}
this.init();}